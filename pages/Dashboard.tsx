...
import { Headphones, Zap, Newspaper, ChevronRight, Bell, BellOff, X, CheckCircle, Clock, Sun, Moon } from 'lucide-react';
...
export const Dashboard: React.FC = () => {
  ...
  // Handle Status Bar for Dashboard
  useEffect(() => {
    if (Capacitor.isNativePlatform()) {
      const setStatus = async () => {
        try {
          if (theme === 'dark') {
            // Dark Mode: Dark Background -> Needs White Icons (Style.Dark)
            await StatusBar.setStyle({ style: Style.Dark });
            await StatusBar.setBackgroundColor({ color: '#0f172a' }); 
          } else {
            // Light Mode: White Background -> Needs Black Icons (Style.Light)
  ...
  return (
    // Fixed container to ensure full screen coverage
    <div className="fixed inset-0 bg-slate-50 dark:bg-slate-900 transition-colors duration-300 overflow-hidden">
      ...
      <button
        ...
        className={`
          absolute left-6 p-2.5 rounded-full shadow-sm border transition-all z-20 flex items-center gap-2 px-4
          ${remindersEnabled 
            ? 'bg-tusgu-blue text-white border-transparent shadow-blue-900/20' 
      ...
      <div className="w-full h-full overflow-y-auto scrollable-container flex flex-col pt-safe-top pb-safe-bottom">
        <div className="flex-1 flex flex-col px-6 pb-12 min-h-full">
          ...
          {/* Dual Logo Header */}
          <div className="flex flex-row items-center justify-center gap-4 md:gap-6 w-full max-w-4xl mx-auto flex-shrink-0 animate-in zoom-in-95 duration-700 mt-20 mb-8 landscape:mt-8 landscape:mb-4">
            <a 
              href="https://www.ucmassrilanka.com/"
              target="_blank"
              rel="noopener noreferrer"
              className="flex-1 flex justify-end transform transition-transform active:scale-95 duration-300"
            >
              <img 
                src={ucmasLogo} 
                alt="UCMAS Sri Lanka" 
                className="h-20 md:h-32 landscape:h-16 object-contain drop-shadow-sm" 
              />
            </a>

            <div className="h-10 md:h-24 landscape:h-12 w-[2px] bg-slate-200 dark:bg-slate-700 rounded-full opacity-60"></div>
          ...
          {/* Menu Grid */}
          <div className="flex-1 w-full max-w-4xl mx-auto px-1 grid grid-cols-1 md:grid-cols-2 landscape:grid-cols-2 gap-5 animate-in slide-in-from-bottom-8 duration-700 pb-20 md:pb-8 content-start md:content-center">
            {menuItems.map((item) => (
              <button
                key={item.title}
                onClick={() => handleNavigate(item.path)}
                className={`
                  relative overflow-hidden group rounded-[2rem] p-6 transition-all duration-300
                  bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-soft hover:shadow-xl hover:-translate-y-1 active:scale-[0.98]
              ...
              >
                ...
                <div className="flex relative z-10 w-full flex-row items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className={`
                      w-14 h-14 md:w-16 md:h-16 rounded-2xl flex items-center justify-center shadow-sm flex-shrink-0
                      ${item.iconBg}
                    `}>
                      <item.icon className="w-7 h-7 md:w-8 md:h-8" />
                    </div>
                    <div className="min-w-0">
                      <h3 className="text-lg md:text-2xl font-bold text-slate-800 dark:text-white group-hover:text-tusgu-blue dark:group-hover:text-blue-300 transition-colors truncate">{item.title}</h3>
                      <p className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-wide mt-1 truncate">{item.subtitle}</p>
                    </div>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-700/50 p-2 md:p-3 rounded-full flex-shrink-0">
                    <ChevronRight className="w-4 h-4 md:w-5 md:h-5 text-slate-300 dark:text-slate-500" />
                  </div>
                </div>
              </button>
            ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white dark:bg-slate-800 w-full max-w-sm rounded-3xl p-6 shadow-2xl animate-in zoom-in-95 duration-300 relative">
                ...
                <div className="text-center mb-6">
                    <div className="w-12 h-12 bg-blue-50 dark:bg-blue-900/30 text-tusgu-blue dark:text-blue-400 rounded-full flex items-center justify-center mx-auto mb-3">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {(['day', 'week', 'month'] as const).map((f) => (
                                <button
                                    key={f}
                                    onClick={() => setFreq(f)}
                                    className={`py-2 rounded-xl text-sm font-bold capitalize transition-all border-2 ${
                                        freq === f 
                                        ? 'border-tusgu-blue bg-blue-50 text-tusgu-blue dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-400' 
                                        : 'border-slate-100 bg-white text-slate-500 dark:bg-slate-700 dark:border-slate-600 dark:text-slate-300'
                                    }`}
                                >
                                    {f === 'day' ? 'Daily' : f === 'week' ? 'Weekly' : 'Monthly'}
                                </button>
                            ))}
                        </div>
                    </div>
                        ...
                        <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">How Many Times?</label>
                         <div className="flex items-center justify-between bg-slate-50 dark:bg-slate-900 rounded-xl p-2 border border-slate-200 dark:border-slate-700">
                             ...
                             <div className="text-lg font-bold text-slate-800 dark:text-white">
                                {count} <span className="text-sm font-normal text-slate-500">per {freq}</span>
                             </div>
                ...
                <button 
                    ...
                    className="w-full py-3 bg-tusgu-blue text-white rounded-xl font-bold text-lg shadow-lg shadow-blue-900/20 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
                >
                    <CheckCircle className="w-5 h-5" /> Turn On
                </button>
            </div>
        </div>
      )}
